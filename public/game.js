/* ══════════════════════════════════════════════════════════════════════════════
   RENDER ROYALE — Client (clean edition)
   ══════════════════════════════════════════════════════════════════════════════ */
...
function productImgUrl(photoId, w = 400, h = 300) {
  ...
  return `https://images.unsplash.com/${photoId}?w=${w}&h=${h}&fit=crop&crop=center&auto=format&q=80`;
}
...
function renderTable(containerId, opts = {}) {
  const {
    ...
    hostId = null, judgeId = null, winnerId = null,
    showScore = false, selectedIds = null,
  } = opts;
...
function renderLobby() {
  ...
  $('#lobby-code').innerHTML = code.split('').map((ch, i) =>
    `<span class="room-code-letter" style="animation-delay: ${i * 0.08}s">${ch}</span>`
  ).join('');

  renderTable('lobby-seats', { hostId: state.hostId });
...
$('#btn-start').addEventListener('click', () => socket.emit('start-game', { rounds }));
...
function showRoundStart(data) {
  ...
  renderTable('round-start-seats', { judgeId: data.judgeId });
  ...
  if (state.soloMode) {
    ...
    if (state.isJudge) {
      $('#judge-label').innerHTML = 'You are the <strong>Judge</strong> this round';
    } else {
...
function showCardSelection() {
  if (state.isJudge && !state.soloMode) { showRenderScreen(); return; }

  renderTable('selection-seats', { judgeId: state.judgeId, selectedIds: selectedPlayers });
  ...
  hand.querySelectorAll('.prompt-card').forEach(el => {
    el.addEventListener('click', () => {
      ...
      if (instr) {
        instr.innerHTML = remaining === 0
          ? 'Looking good! Hit <strong>Lock In</strong> when ready'
          : `Pick <strong>${remaining} more card${remaining > 1 ? 's' : ''}</strong> to define your render`;
      }
    });
  });
...
$('#btn-lock-cards').addEventListener('click', () => {
  ...
  $('#player-hand').querySelectorAll('.prompt-card').forEach(el => el.classList.add('locked'));
  $('#btn-lock-cards').disabled = true;
...
function showMySelectedCards(selections) {
  ...
  $('#render-my-cards').innerHTML = '<p class="label" style="margin-bottom:8px">YOUR PROMPT CARDS</p>' +
    myCards.map(card => `
...
function showJudging(data) {
  ...
  state.pickedSubmission = null;
  renderTable('judging-seats', { judgeId: data.judgeId });
  ...
  grid.innerHTML = data.submissions.map((sub, i) => {
    ...
    return `
      <div class="submission-card ${state.isJudge ? '' : 'not-judge'}" data-index="${sub.index}" style="animation-delay: ${i * 0.1}s">
        ${imgHtml}
...
function showScoreboard(data) {
  state.players = data.scores.map(s => ({ ...state.players.find(p => p.id === s.id), ...s }));

  $('#winner-name').textContent = data.soloMode ? 'Nice render!' : data.winnerName;
  if (!data.soloMode) launchConfetti();

  renderTable('scoreboard-seats', { winnerId: data.winnerId, showScore: true });

  const sorted = [...data.scores].sort((a, b) => b.score - a.score);
  $('#scoreboard-center').innerHTML = `
...
function showGameOver(data) {
  ...
  $('#final-scores').innerHTML = sorted.map((p, i) => {
    ...
    return `
      <div class="final-row ${i === 0 ? 'champion' : ''}" style="animation-delay: ${i * 0.1}s">
...
socket.on('room-created', (data) => {
  state.myId = data.you; state.roomCode = data.code; state.players = data.players;
  state.hostId = data.you; state.isHost = true;
  renderLobby(); showScreen('lobby');
});

socket.on('room-joined', (data) => {
  state.myId = data.you; state.roomCode = data.code; state.players = data.players;
  state.hostId = data.hostId; state.isHost = data.hostId === data.you;
  renderLobby(); showScreen('lobby');
});

socket.on('player-joined', (data) => {
  state.players = data.players; renderLobby();
  showToast(`${data.player.name} joined!`, 'success');
});

socket.on('player-left', (data) => {
  state.players = data.players; state.hostId = data.hostId;
  state.isHost = data.hostId === state.myId; renderLobby();
});

socket.on('error-msg', (data) => showToast(data.message));
socket.on('game-started', (data) => { state.totalRounds = data.totalRounds; });

socket.on('round-started', (data) => {
  state.players = data.players; submittedPlayers.clear(); showRoundStart(data);
});

socket.on('phase-change', (data) => {
  if (data.phase === 'card-selection') showCardSelection();
  else if (data.phase === 'rendering') showRenderScreen();
});

socket.on('all-selected', (data) => showMySelectedCards(data.selections));
socket.on('auto-selected', (data) => {
  state.selected = data.cards.map(c => c.id);
  showToast('Time\u2019s up! Cards auto-selected.', 'info');
});
...
socket.on('player-submitted', (data) => {
  submittedPlayers.add(data.playerId); updateRenderStatus(submittedPlayers);
});
...
socket.on('back-to-lobby', (data) => { state.players = data.players; renderLobby(); showScreen('lobby'); });
...
function launchConfetti() {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    piece.style.width = piece.style.height = (5 + Math.random() * 7) + 'px';
